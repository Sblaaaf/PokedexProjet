...
def index(request):
    ...
    if current_pk:
        # On limite la recherche aux 150 premiers Pokémon
        if current_pk['id'] > 150:
            context['error'] = "Limited search to Pokémon #1 to #150."
...
# COMBAT V1 - ancienne version basée sur "power"
""" def combat(request):
    # Equipe choisie
    ...
    # recuperation des données de l'IA si déjà générée
    if 'ai_team' not in request.session:
        for _ in range(5):
            rand_id = random.randint(1, 151)
            response = requests.get(f"{API_URL}{rand_id}")
            data = response.json()
            ai_pokemon = {
                'name': data['name'],
                'image': data['sprites']['other']['official-artwork']['front_default'] or data['sprites']['front_default'],
    ...
    # fight ! On va stocker le résultat de chaque duel, on compare les listes
    results = []
    player_score = 0
    ai_score = 0

    # On boucle 5 fois (zip permet de prendre le 1er des deux listes, puis le 2eme...)
    for player_pk, ai_pk in zip(player_team, ai_team):
        if player_pk['power'] > ai_pk['power']:
            winner = 'player'
            player_score += 1
        else:
            winner = 'ai'
    ...
    if player_score > ai_score:
        final_result = "VICTOIRE !"
        color = "green"
    else:
        final_result = "DÉFAITE..."
        color = "red"
